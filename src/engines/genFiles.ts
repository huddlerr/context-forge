...
export function genFiles(answers: string[]): Record<string, string> {
  ...
  const pkg = JSON.stringify(
    {
      ...
      private: true,
      scripts,
      dependencies: stk.deps,
      devDependencies: stk.devDeps,
    },
    null,
    2,
  );
  ...
  const tsconf = JSON.stringify(
    {
      compilerOptions: {
        strict: true,
        target: "expo" in stk.detected ? "ESNext" : "ES2022",
        module: "ESNext",
  ...
  if (auth.length) {
    envL.push("# Auth");
    auth.forEach((p) => p.k.forEach((k) => envL.push(`${k}=`)));
    envL.push("");
  }
  if (ints.length) {
    envL.push("# Integrations");
    ints.forEach((ig) => {
      envL.push(`# ${ig.n}`);
      ig.k.forEach((k) => envL.push(`${k}=`));
    });
    envL.push("");
  }
  ...
  if (ents.length) {
    ents.forEach((e) => {
      ...
      e.fields.forEach((f) => {
        const t = f.name.includes("_id")
          ? "string"
          : f.name.includes("_at") || f.name.includes("date")
            ? "Date"
            : f.name.includes("amount") || f.name.includes("price")
              ? "number"
              : f.name.includes("is_")
                ? "boolean"
                : "string";
  ...
  return {
    ...
    "claude.md": claude,
    "package.json": pkg,
    "tsconfig.json": tsconf,
    ".env.example": envL.join("\n"),
    "types.ts": types,
    "README.md": readme,
    "TASKS.md": tasks,
    ".cursorrules": rules,
